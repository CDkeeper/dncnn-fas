...
test_dataset = MY_MNIST_Test(root=r'C:\Users\chenda\Desktop\Fashion_1on1_dncnn_20000_5_128\test-GI-pt/1.pt')

test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=False)
model = torch.load("./model_weight/module_5.pth").to(device)
# print(model)
print("模型加载成功！")
...
for batch_idx, data in enumerate(test_loader):
    ...
    for i in range(batch_size):  # 读取每一批图片，并分离
        img = output[i]
        save_image(img, r'C:\Users\chenda\Desktop\Fashion_1on1_dncnn_20000_5_128\test-result/' + str(i + 1) + '.jpg'.format(
